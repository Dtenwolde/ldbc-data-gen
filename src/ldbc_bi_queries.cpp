...
static string LdbcSQLIdentifier(const string &identifier) {
...
static string PragmaLdbcBiQuery(ClientContext &context, const FunctionParameters &parameters) {
	...
	string catalog = default_entry.catalog;
	string schema = default_entry.schema;
	if (catalog.empty()) {
	...
	const auto dataset = LdbcSQLIdentifier(catalog) + "." + LdbcSQLIdentifier(schema);
