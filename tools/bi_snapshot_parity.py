#!/usr/bin/env python3
"""Compare all DuckDB-generated BI initial snapshot relations with Spark reference output."""
...
def canonical_expr(relation: dict, source: str) -> str:
    return (
        ...
        f"md5(coalesce(string_agg({row_expr(relation)}, '{ROW_SEP}' ORDER BY {order_by_expr(relation)}), '')) "
        f"AS checksum FROM {source}"
    )
...
def create_reference_view(duckdb: Path, database: Path, reference_root: Path, relation: dict, fmt: str) -> None:
    ...
    if fmt == "parquet":
        scan = f"read_parquet({sql_string(path / '**/*.parquet')}, union_by_name = true)"
    else:
        columns = ", ".join(f"'{column['name']}': '{column['type']}'" for column in relation["columns"])
        scan = f"read_csv({sql_string(path / '**/*.csv*')}, delim = '|', header = true, columns = {{{columns}}})"
...
def first_diff_sql(relation: dict, left_source: str, right_source: str) -> str:
    ...
    return (
        f"SELECT {left_columns} FROM {left_source} l "
        f"LEFT JOIN {right_source} r ON {left_keys} "
        f"WHERE {null_check} ORDER BY {order_by_expr(relation, 'l')} LIMIT 1"
    )
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--schema", default="ldbc_bi_snapshot_parity")
    ...
    with tempfile.TemporaryDirectory(prefix="ldbc-bi-snapshot-parity-") as tmpdir:
        ...
        for relation in relations:
            generated_source = f"{quote_ident(args.schema)}.{quote_ident(relation['name'])}"
    ...
    if diffs:
        writer.writerow([])
        writer.writerow(["relation", "side", "first_diff_columns..."])
        with tempfile.TemporaryDirectory(prefix="ldbc-bi-snapshot-diff-") as tmpdir:
            database = Path(tmpdir) / "diff.duckdb"
            setup_sql = (
                f"LOAD {sql_string(args.extension)}; "
                f"CALL ldbcgen(sf := {args.sf}, schema := {sql_string(args.schema)}, overwrite := true);"
            )
            ...
            for relation, generated_source, reference_source in diffs:
                create_reference_view(args.duckdb, database, args.reference, relation, args.format)
                print_first_diff(writer, args.duckdb, database, relation, generated_source, reference_source, "generated_only")
                print_first_diff(writer, args.duckdb, database, relation, reference_source, generated_source, "reference_only")
